...
if rank == 0:
    lat_dataframe = pd.DataFrame(index = lat_size, columns = [f'MPI: {size} core'], data = lat_time)
    iter_dataframe = pd.DataFrame(index = iter_size, columns = [f'MPI: {size} core'], data = iter_time)

    lat_dataframe.to_csv(f'./times/lat_mpi_{size}_core.csv')
    iter_dataframe.to_csv(f'./times/iter_mpi_{size}_core.csv')